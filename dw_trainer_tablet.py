...
# ==============================================================================
# 📱 ANDROID / TABLET ENVIRONMENT SETUP
# ==============================================================================
...
if os.name == 'nt':
    try:
        import ctypes
        appid = 'ev_navigator.deuces_wild.tablet.v9_5'
...
# ==============================================================================
# ⚙️ TABLET CONFIGURATION & SCALING
# ==============================================================================
# Native Logic Resolution: 1600 x 850
# Target Tablet Resolution: 2560 x 1600

# DETECT ENVIRONMENT
IS_ANDROID = "ANDROID_ARGUMENT" in os.environ

if IS_ANDROID:
    # 📱 PIXEL TABLET MODE (1.6x Scale)
    SCALE = 1.6      
    Y_OFFSET = 120   # Centering for 16:10 aspect ratio
    FULLSCREEN_FLAG = pygame.FULLSCREEN
    FPS_LIMIT = 30   # Battery Saver
else:
    # 💻 PC DEBUG MODE (1.0x Scale - Native)
    SCALE = 1.0      # 1600px Wide (Fits standard 1080p monitors)
    Y_OFFSET = 0     # No padding needed for PC window
    FULLSCREEN_FLAG = pygame.RESIZABLE
    FPS_LIMIT = 60   # Smooth on PC

def s(val):
    """Global Scaler: Integers only"""
    return int(val * SCALE)

def s_rect(x, y, w, h):
    """Creates a scaled Rect with vertical centering applied."""
    return pygame.Rect(s(x), s(y) + Y_OFFSET, s(w), s(h))

def s_font(size):
    """Scales font size."""
    return int(size * SCALE)

# Calculate Screen Dimensions based on the dynamic SCALE
SCREEN_W = s(1600)
SCREEN_H = s(850) + (Y_OFFSET * 2) 

# If on Android, force exact hardware specs to be safe
if IS_ANDROID:
...
# Path to card images (Tablet High-Res)
ASSET_DIR = os.path.join("images", "cards")
SOUND_DIR = "sounds"
# Scaled Card Size: 142*SCALE, 215*SCALE
...
class AssetManager:
    def __init__(self):
        ...
        # Fallback fonts for Android if Segoe UI is missing
        try:
...
class StrategyPanel:
    def __init__(self, x, y, w, h, assets, machine):
        # Apply scaling and offset to the main rect
        self.rect = s_rect(x, y, w, h)
        self.assets = assets
        self.machine = machine
        self.results = [] 
        
        # Setup button position relative to scaled panel
        cx = self.rect.centerx
        # We manually scale the layout relative to the panel's new coordinates
...
class CodexScreen:
    ...
    def draw(self, screen):
        s_surf = pygame.Surface((SCREEN_W, SCREEN_H), pygame.SRCALPHA)
...
class LogPanel:
    ...
    def handle_event(self, event):
        ...
        # Touch Scroll Support (FINGERMOTION) could be added here, but Drag Logic covers it
...
class ClickableMeter:
    def __init__(self, x_center, y_base, label, color, default_is_credits=True):
        self.x_center = s(x_center); self.y_base = s(y_base) + Y_OFFSET
...
class IGT_Machine:
    def __init__(self):
        ...
        # TABLET MODE: Fullscreen preferred on Android, Resizable on PC
        self.screen = pygame.display.set_mode((SCREEN_W, SCREEN_H), FULLSCREEN_FLAG)
             
        pygame.display.set_caption("EV Navigator - Tablet Edition (v9.5)")
    ...
    def _init_buttons(self):
        ...
        # Button Rects created directly scaled
        self.buttons = [
    ...
    def run_solver(self):
        # 1. Always calculate math for the Strategy Panel (The List)
        results = dw_fast_solver.solve_hand(self.hand, self.sim.paytable)
        self.strategy_panel.set_results(results)
        
        # 2. Reset visuals (Clear old Gold/Silver markers)
        ...
        if self.auto_hold_active:
            ...
            # If Auto Hold is OFF, ensure we start with a clean slate (User must click)
            self.held_indices = []
    ...
    def _draw_main_game_elements(self):
        ...
        cx, cy = s(910), s(680) + Y_OFFSET + s(30); rect = pygame.Rect(cx-s(40), cy-s(25), s(80), s(50))
